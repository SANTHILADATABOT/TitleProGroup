...
{
    public function als_view(Request $request)
    {
        $list_the_als = $request->input('list_the_als');
        $filter = $request->input('filter');
        $power = $request->input('power');
        $status_1 = $request->input('status_1');
        $overload = $request->input('overload');

        if ($power && $status_1 && $overload != null) {
            return response()->json(['status' => 'SUCCESS', 'message' => 'View Showed Successfully', 'list_the_als' => $list_the_als, 'filter' => $filter, 'power' => $power, 'status_1' => $status_1, 'overload' => $overload], 200);
        } else {
            return response()->json(['status' => 'FAILURE', 'message' => 'View Not Found'], 404);
        }
    }
