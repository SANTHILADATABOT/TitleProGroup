...
const TasksReport = () => {
  ...
  const fetch_data = async () => {
    var response1 = await ReportsRepository.tasks_report_index({});
    if (response1) {
      if (response1?.status === "SUCCESS") {
        var table_content_div1 = [];
        (response1?.tasks_report_index || []).forEach(
          (element, index) => {
      ...
      <div className="container-fluid">
        <div className="page-header">
          <div className="row">
            <div className="col-lg-6 main-header">
              <h2>
                Tasks <span style={{ color: "black" }}>Report</span>
              </h2>
            </div>
        ...
        <div className="row">
          ...
          {loading ? (
              ...
              <DataTableComponent column={table_col} row_data={table_content_div} exportColumns={[0,1,2,3,4,5,6,7,8]} exportBtns={{excel:true,pdf:true}} exportTitle="Tasks Report" />
            )}
          </div>
        </div>
      </div>      
