...
if ((user_rights?.edit_rights === "1") || (user_rights?.delete_rights === "1")) {
  ...
  const fetch_data = async () => {
    var response1 = await DataRepository.expense_type_creation_index({});
    if (response1) {
      if (response1?.status === "SUCCESS") {
        var table_content_div1 = [];
        (response1?.expense_type_creation_index || []).forEach(
          (element, index) => {
            var tb_row = [(<span onClick={() => {handleCopy(element);}}>{index + 1}</span>),element?.entry_date || "", element?.expense_type_name || "", element?.description || "", (<span className={`badge badge-${element?.status === "active" ? "success" : "danger"}`}>{element?.status || ""}</span>),];
            if((user_rights?.edit_rights === "1") || (user_rights?.delete_rights === "1")){
              tb_row.push(<span className="text-center">
                {(user_rights?.edit_rights === "1") && (
                  <button
                    ...
                    onClick={() =>
                      openInputFormModal(element?.expense_type_id || "")
                    }
                  >
                ...
                {(user_rights?.delete_rights === "1") && (
                  <button
                    ...
                    onClick={() =>
                      deleteDataRow(element?.expense_type_id || "")
                    }
                  >
  ...
  const openInputFormModal = async (id) => {
    ...
    if (id === "") {
      $("#inputFormModal #expensetypeModalLabel").html(
        "Add Expense Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Add Expense Type Creation");
    } else {
      $("#inputFormModal #expensetypeModalLabel").html(
        "Edit Expense Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Update Expense Type Creation");
      var response1 = await DataRepository.expense_type_creation_edit({
        expense_type_id: id,
      });
      if (response1) {
        if (response1?.status === "SUCCESS") {
          var edit_data = response1?.expense_type_creation_edit || [];
          if (edit_data.length > 0) {
            setFormData({
              expense_type_id: id,
              expense_type_name: edit_data[0]?.expense_type_name || "",
  ...
  const submitDataRow = async (e) => {
    ...
    if (formData?.expense_type_id === "") {
      response1 = await DataRepository.expense_type_creation_insert(formData);
    } else {
      response1 = await DataRepository.expense_type_creation_update(formData);
    }
    if (response1) {
      if (response1?.status === "SUCCESS") {
        refreshTable();
  ...
  const deleteDataRow = async (id) => {
    if (id !== "") {
      Swal.fire({
        title: "Remove Expense Type Creation",
        ...
        if (result.isConfirmed) {
          const get_init_datas = async () => {
            var response1 = await DataRepository.expense_type_creation_delete({
              expense_type_id: id,
            });
            if (response1) {
              if (response1?.status === "SUCCESS") {
  ...
  return (
    <>
        ...
        <div className="page-header">
          <div className="row">
            <div className="col-lg-6 main-header">
              {/* <h2>Expense Type Creation</h2> */}
              <h2>
                Expense Type<span style={{ color: "black" }}>Creation</span>
              </h2>
            </div>
            <div
              className="col-lg-6"
            ...
            >
              ...
              {user_rights?.add_rights === "1" && (
                ...
                >
                  <i className="fa fa-plus text-white"></i> Add Expense Type
                  Creation
                </button>
              )}
            </div>
        ...
        <div className="row">
          <div className="col-sm-12">
            {loading ? (
              ...
              <DataTableComponent column={table_col} row_data={table_content_div} exportColumns={[0,1,2,3,4]} exportBtns={{excel:(user_rights?.more_rights?.["Excel"] === "1"),pdf:true}} exportTitle="Expense Type Creation" />
            )}
          </div>
        </div>
      </div>
      <div
        ...
        tabIndex="-1"
        role="dialog"
        aria-labelledby="expensetypeModalLabel"
        aria-hidden="true"
      >
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="expensetypeModalLabel">
                Modal title
              </h5>
              <button
                className="btn-close"
                ...
                <div className="form-row">
                  <div className="col-md-12 mb-3">
                    <label className="form-label">
                      Expense Type Name <span className="text-danger">*</span>
                    </label>
                    <input
...
export default ExpenseTypeCreation;
