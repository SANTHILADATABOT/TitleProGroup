...
const ContactTypeCreation = () => {
  ...
  const handleCopy = (item) => {
    ...
    const textToCopy =
      ...
      `Contact Type Name: ${item?.contact_type_name || ""}\n` +
  ...
  const fetch_data = async () => {
    var response1 = await DataRepository.contact_type_creation_index({});
    if (response1) {
      if (response1?.status === "SUCCESS") {
        var table_content_div1 = [];
        (response1?.contact_type_creation_index || []).forEach(
          (element, index) => {
            var tb_row = [(<span onClick={() => {handleCopy(element);}}>{index + 1}</span>), element?.entry_date || "", element?.contact_type_name || "", (<div style={{width:"300px",display:"inline-block"}}>{element?.description}</div>) || "", (<span className={`badge badge-${element?.status === "active" ? "success" : "danger"}`}>{element?.status || ""}</span>)];
            if((user_rights?.edit_rights === "1") || (user_rights?.delete_rights === "1")){
              tb_row.push(<span className="text-center">
                {user_rights?.edit_rights === "1" && (
                  <button
                    ...
                    onClick={() =>
                      openInputFormModal(element?.contact_type_id || "")
                    }
                  >
                ...
                {user_rights?.delete_rights === "1" && (
                  <button
                    ...
                    onClick={() =>
                      deleteDataRow(element?.contact_type_id || "")
                    }
                  >
  ...
  const openInputFormModal = async (id) => {
    ...
    if (id === "") {
      $("#inputFormModal #contactTypeModalLabel").html(
        "Add Contact Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Add Contact Type Creation");
    } else {
      $("#inputFormModal #contactTypeModalLabel").html(
        "Edit Contact Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Update Contact Type Creation");
      var response1 = await DataRepository.contact_type_creation_edit({
        contact_type_id: id,
      });
      if (response1) {
        if (response1?.status === "SUCCESS") {
          var edit_data = response1?.contact_type_creation_edit || [];
  ...
  const submitDataRow = async (e) => {
    ...
    if (formData?.contact_type_id === "") {
      response1 = await DataRepository.contact_type_creation_insert(formData);
    } else {
      response1 = await DataRepository.contact_type_creation_update(formData);
    }
    if (response1) {
      if (response1?.status === "SUCCESS") {
        refreshTable();
  ...
  const deleteDataRow = async (id) => {
    if (id !== "") {
      Swal.fire({
        title: "Remove Contact Type Creation",
        ...
        if (result.isConfirmed) {
          const get_init_datas = async () => {
            var response1 = await DataRepository.contact_type_creation_delete({
              contact_type_id: id,
            });
            if (response1) {
              if (response1?.status === "SUCCESS") {
  ...
  return (
    <>
        ...
        <div className="page-header">
          <div className="row">
            <div className="col-lg-6 main-header">
              <h2>
                Contact Type <span>Creation</span>
              </h2>
            </div>
            <div
              className="col-lg-6"
            ...
            >
              ...
              {user_rights?.add_rights === "1" && (
                ...
                >
                  <i className="fa fa-plus text-white"></i> Add Contact Type
                  Creation
                </button>
              )}
            </div>
      ...
      <div
        ...
        tabIndex="-1"
        role="dialog"
        aria-labelledby="contactTypeModalLabel"
        aria-hidden="true"
      >
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="contactTypeModalLabel">
                Modal title
              </h5>
              <button
                className="btn-close"
...
export default ContactTypeCreation;
