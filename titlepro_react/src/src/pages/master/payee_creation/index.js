...
const PayeeCreation = () => {
  ...
  const fetch_data = async () => {
    var response1 = await DataRepository.payee_creation_index({});
    if (response1) {
      if (response1?.status === "SUCCESS") {
        var table_content_div1 = [];
        (response1?.payee_creation_index || []).forEach(
          (element, index) => {
            var tb_row = [(<span onClick={() => { handleCopy(element); }}>{index + 1}</span>), element?.entry_date || "", element?.payee_name || "" || "", element?.description || "", (<span className={`badge badge-${element?.status === "active" ? "success" : "danger"}`}>{element?.status || ""}</span>),];
            if ((user_rights?.edit_rights === "1") || (user_rights?.delete_rights === "1")) {
              tb_row.push(<span className="text-center">
                ...
                {(user_rights?.delete_rights === "1") && (
                  <button
                    ...
                    onClick={() =>
                      deleteDataRow(element?.payee_id || "")
                    }
                  >
  ...
  const openInputFormModal = async (id) => {
    ...
    if (id === "") {
      $("#inputFormModal #payeeModalLabel").html("Add Payee Creation");
      $("#inputFormModal #ip_submit_btn").html("Add Payee Creation");
    } else {
      $("#inputFormModal #payeeModalLabel").html("Edit Payee Creation");
      $("#inputFormModal #ip_submit_btn").html("Update Payee Creation");
      var response1 = await DataRepository.payee_creation_edit({
        payee_id: id,
      });
      if (response1) {
        if (response1?.status === "SUCCESS") {
          var edit_data = response1?.payee_creation_edit || [];
          if (edit_data.length > 0) {
            setFormData({
              payee_id: id,
              payee_name: edit_data[0]?.payee_name || "",
  ...
  const submitDataRow = async (e) => {
    ...
    if (formData?.payee_id === "") {
      response1 = await DataRepository.payee_creation_insert(formData);
    } else {
      response1 = await DataRepository.payee_creation_update(formData);
    }
    if (response1) {
      if (response1?.status === "SUCCESS") {
        refreshTable();
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const get_init_datas = async () => {
            var response1 = await DataRepository.payee_creation_delete({
              payee_id: id,
            });
            if (response1) {
              if (response1?.status === "SUCCESS") {
  ...
  return (
    <>
        ...
        <div className="page-header">
          <div className="row">
            <div className="col-lg-6 main-header">
              <h2>
                Payee <span style={{ color: "black" }}>Creation</span>
              </h2>
            </div>
            <div
              className="col-lg-6"
            ...
            >
              ...
              {user_rights?.add_rights === "1" && (
                ...
                >
                  <i className="fa fa-plus text-white"></i> Add Payee
                  Creation
                </button>
              )}
            </div>
        ...
        <div className="row">
          <div className="col-sm-12">
            {loading ? (
              ...
              <DataTableComponent column={table_col} row_data={table_content_div} exportColumns={[0, 1, 2, 3, 4]} exportBtns={{ excel: (user_rights?.more_rights?.["Excel"] === "1"), pdf: true }} exportTitle="Payee Creation" />
            )}
          </div>
        </div>
      </div>
      <div
        ...
        tabIndex="-1"
        role="dialog"
        aria-labelledby="payeeModalLabel"
        aria-hidden="true"
      >
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="payeeModalLabel">
                Modal title
              </h5>
              <button
                className="btn-close"
