...
const AssignTypeCreation = () => {
  ...
  const handleCopy = (item) => {
    ...
    const textToCopy = `Entry Date: ${
      item.entry_date || ""
    }\nAssign Type Name: ${item.assign_type_name || ""}\nDescription: ${
      item.description || ""
    }\nStatus: ${item.status || ""}`;
  ...
  const fetch_data = async () => {
    var response1 = await DataRepository.assign_type_creation_index({});
    if (response1) {
      if (response1?.status === "SUCCESS") {
        var table_content_div1 = [];
        (response1?.assign_type_creation_index || []).forEach(
          (element, index) => {
            var tb_row = [(<span onClick={() => {handleCopy(element);}}>{index + 1}</span>), element?.entry_date || "", element?.assign_type_name || "", element?.description || "", (<span className={`badge badge-${element?.status === "active" ? "success" : "danger"}`}>{element?.status || ""}</span>), ];
              ...
              tb_row.push(<span className="text-center">
                {(user_rights?.edit_rights === "1") && (
                  <button
                    ...
                    onClick={() =>
                      openInputFormModal(element?.assign_type_id || "")
                    }
                  >
                ...
                {(user_rights?.delete_rights === "1") && (
                  <button
                    ...
                    onClick={() =>
                      deleteDataRow(element?.assign_type_id || "")
                    }
                  >
  ...
  const openInputFormModal = async (id) => {
    ...
    if (id === "") {
      $("#inputFormModal #assigntypeModalLabel").html(
        "Add Assign Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Add Assign Type Creation");
    } else {
      $("#inputFormModal #assigntypeModalLabel").html(
        "Edit Assign Type Creation"
      );
      $("#inputFormModal #ip_submit_btn").html("Update Assign Type Creation");
      var response1 = await DataRepository.assign_type_creation_edit({
        assign_type_id: id,
      });
      if (response1) {
        if (response1?.status === "SUCCESS") {
          var edit_data = response1?.assign_type_creation_edit || [];
          if (edit_data.length > 0) {
            setFormData({
              assign_type_id: id,
              assign_type_name: edit_data[0]?.assign_type_name || "",
  ...
  const submitDataRow = async (e) => {
    ...
    var response1 = null;
    if (formData?.assign_type_id === "") {
      response1 = await DataRepository.assign_type_creation_insert(formData);
    } else {
      response1 = await DataRepository.assign_type_creation_update(formData);
    }
    if (response1) {
      if (response1?.status === "SUCCESS") {
        refreshTable();
  ...
  const deleteDataRow = async (id) => {
    if (id !== "") {
      Swal.fire({
        title: "Remove Assign Type Creation",
        ...
        if (result.isConfirmed) {
          const get_init_datas = async () => {
            var response1 = await DataRepository.assign_type_creation_delete({
              assign_type_id: id,
            });
            if (response1) {
              if (response1?.status === "SUCCESS") {
  ...
  return (
    <>
      <div className="container-fluid">
        <div className="page-header">
          <div className="row">
            <div className="col-lg-6 main-header">
              <h2>
                Assign Type <span style={{ color: "black" }}>Creation</span>
              </h2>
            </div>
            <div
              className="col-lg-6"
            ...
            >
              ...
              {user_rights?.add_rights === "1" && (
                ...
                >
                  <i className="fa fa-plus text-white"></i> Add Assign Type
                  Creation
                </button>
              )}
            </div>
        ...
        <div className="col-sm-12">
            {loading ? (
              ...
              <DataTableComponent column={table_col} row_data={table_content_div} exportColumns={[0,1,2,3,4]} exportBtns={{excel:(user_rights?.more_rights?.["Excel"] === "1"),pdf:true}} exportTitle="Assign Type Creation" />
            )}
          </div>
        </div>
      </div>
      <div
        ...
        tabIndex="-1"
        role="dialog"
        aria-labelledby="assigntypeModalLabel"
        aria-hidden="true"
      >
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="assigntypeModalLabel">
                Modal title
              </h5>
              <button
                className="btn-close"
                ...
                <div className="form-row">
                  <div className="col-md-12 mb-3">
                    ...
                    <input
                      ...
                      value={formData?.assign_type_name}
                      onChange={handleChange}
                      type="text"
                      placeholder="Enter Assign Type Name"
                      required
                    />
                  </div>
                  <div className="col-md-12 mb-3">
...
export default AssignTypeCreation;
