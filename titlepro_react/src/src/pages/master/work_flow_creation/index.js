...
const WorkFlowCreation = () => {
    ...
    if (response1 && response1.status === "SUCCESS") {
      ...
      (response1?.work_flow_creation_index || []).forEach((element) => {
        ...
        (element?.task_creation_index || []).forEach((taskElement, index) => {
          var tb_row = [(<span onClick={() => {handleCopy(taskElement);}}>{index + 1}</span>), taskElement?.task_name || "", taskElement?.specific_task || "", (<span className={`badge badge-${element?.status === "active" ? "success" : "danger"}`}>{element?.status || ""}</span>),];
          ...
          <>
            <div className="row" key={element.work_flow_id}>
              ...
              {loading ? (
                ...
                <DataTableComponent column={tableCol} row_data={tableContentDiv1} exportColumns={[0,1,2,3,4]} exportBtns={{excel:true,pdf:true}} exportTitle="Contact Creation" />
              )}
              </div>
            </div>
            <br />
  ...
  const submitWorkFlowDataRow = async (e) => {
    ...
    if (response1 && response1.status === "SUCCESS") {
      refreshTable();
      $("#inputWorkFlowFormModal .btn-close").trigger({ type: "click" });
    }
  };
  ...
  const submitTaskDataRow = async (e) => {
    ...
    if (response1) {
      if (response1?.status === "SUCCESS") {
        refreshTable();
        $("#inputTaskFormModal .btn-close").trigger({ type: "click" });
      }
    }
  };
