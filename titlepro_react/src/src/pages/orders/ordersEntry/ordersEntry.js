...
//  var [form_load, set_form_load] = useState(true);
 ...
 const [dataBorrowerSource, setBorrowerDataSource] = useState([]);
  ...
  const fetch_borrower_data = async () => {
    try {
      const response1 =
        await OrdersRepository.order_entry_borrower_or_seller_index({
          order_id: order_id,
      ...
      if (response1 && response1.status === "SUCCESS") {
        const newDataSource = (
          response1?.order_borrower_or_seller_index || []
        ).map((element, index) => ({
          key: index,
  ...
  useEffect(() => {
    if (stateId) {
      const fetch_data = async () => {
        try {
          const response1 = await OrdersRepository.order_entry_state_dependency(
            {
              state_id: stateId,
            }
          );
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const get_init_datas = async () => {
            var response1 =
              await OrdersRepository.order_entry_borrower_or_seller_delete({
  ...
  const itemsTab = [
    {
      ...
      children: (
        <BorrowerTable
          dataSource={dataBorrowerSource}
          openDeleteFormModal={openDeleteFormModal}
          openInputFormModal={openInputFormModal}
        />
      ),
    },
    {
  ...
  const submitBorrowerOrSellerDataRow = async (e) => {
    ...
    if (formBorrowerOrSellerData?.order_borrower_or_seller_id === "") {
      response1 = await OrdersRepository.order_entry_borrower_or_seller_insert(
        formData,
      ...
      response1 = await OrdersRepository.order_entry_borrower_or_seller_update(
        formBorrowerOrSellerData
      );
      if (response1) {
        if (response1?.status === "SUCCESS") {
  ...
  const onChange = (newActiveKey) => {
    navigate(`/order-entry/${newActiveKey}/order-entry`);
                ...
                <div className="form-row">
                  ...
                  <div className="col-md-12 mb-3 d-flex flex-column">
                    ...
                    <DatePicker
                      ...
                      onChange={(date) =>
                        setBorrowerOrSellerFormData((prev) => ({
                          ...prev,
                          dob: date,
                        }))
                      }
